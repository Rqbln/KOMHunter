...
│   │       ├── segments.py   /api/segments: explore, geocode, {id}
│   │       ├── athletes.py   /api/athletes: me, me/stats, me/koms, me/starred, me/prs
...
│   │   └── scoring.py        Difficulty index (slope, distance, KOM speed)
...
└── src/
    ├── app/                  App router: layout.tsx, page.tsx (main hunt page)
    ├── components/           layout/ (Header, Sidebar, MainContent), hunt/ (form),
    │                         map/ (SegmentMap, Leaflet), segments/ (table + detail
    │                         panel), dashboard/ (stats, KOMs), ui/
    ├── hooks/                useStrava (auth state), useSegments, useAthleteStats
